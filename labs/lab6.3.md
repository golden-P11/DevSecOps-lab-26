  ...
  checkov:
    ...
    steps:
      ...
      - name: Generate human-readable summary
        if: always()
        run: |
          bash labs/lab6/scripts/summarize_iac_report.sh checkov \
            "${RESULTS_DIR}/checkov-terraform/results_json.json" \
            "${RESULTS_DIR}/checkov-terraform/summary.txt"
  ...
  kics-ansible:
    ...
    steps:
      ...
      - name: Generate human-readable summary
        if: always()
        run: |
          bash labs/lab6/scripts/summarize_iac_report.sh kics \
            "${RESULTS_DIR}/kics-ansible/results.json" \
            "${RESULTS_DIR}/kics-ansible/summary.txt"
  ...
  kics-pulumi:
    ...
    steps:
      ...
      - name: Generate human-readable summary
        ...
        run: |
          bash labs/lab6/scripts/summarize_iac_report.sh kics \
            "${RESULTS_DIR}/kics-pulumi/results.json" \
            "${RESULTS_DIR}/kics-pulumi/summary.txt"
...
```bash
git add .github/workflows/lab6-iac-scanning.yml labs/lab6/scripts/summarize_iac_report.sh
...
5. Download the artifacts — each set should contain JSON/SARIF **and** a `summary.txt` with severity counts and top findings

> **Note:** Checkov combines `--soft-fail` with `--hard-fail-on CRITICAL,HIGH`; KICS uses `--fail-on critical,high`. Upload steps use `if: always()` so JSON, SARIF, and `summary.txt` are saved regardless of scan exit code. The summary is generated by `labs/lab6/scripts/summarize_iac_report.sh` using `jq`.
...
#### Step: Generate human-readable summary
What does `summarize_iac_report.sh` produce? Why generate `summary.txt` in CI instead of reading raw JSON?
...
#### Step: Generate human-readable summary
What statistics appear in `summary.txt` for KICS (severity breakdown, top queries)?
...
- [ ] Task 4 — lab6-iac-scanning.yml + summarize_iac_report.sh committed
...
### Task 4 (4 pts)
- ✅ `.github/workflows/lab6-iac-scanning.yml` and `labs/lab6/scripts/summarize_iac_report.sh` exist; artifacts include JSON/SARIF plus `summary.txt`
